...
Gemma 4 emits tool calls inline in the assistant's content using a
custom token format that is NOT JSON:

    <|tool_call>call:fn_name{key:<|"|>value<|"|>, num:42}<tool_call|>

There are two reasons to parse it client-side instead of relying on
the runtime's `--tool-call-parser gemma4`:

1. **Concurrency safety.** vLLM 0.19.1's parser holds shared mutable
   state across requests; under concurrent tool-call workloads (a
   corpus generator runs 20-64 way) it leaks `<pad>` tokens into the
   response (vLLM #39392). Disabling the server-side parser and parsing
   client-side dodges the bug entirely.
2. **Train/serve shape parity.** An SFT run trains on exactly these raw
...
delimiters `<|"|>` for string values and `<|tool_call>` / `<tool_call|>`
for the call boundary are model-specific. Don't try to use a generic
JSON parser — the inner format isn't JSON.
"""
